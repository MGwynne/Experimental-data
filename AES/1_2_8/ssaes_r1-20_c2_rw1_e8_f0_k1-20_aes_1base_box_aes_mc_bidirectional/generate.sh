...
# Generating 400 key discovery instances for AES(r,1,2,8) using the 1base
...
# All files are in:
#
# ssaes_r1-20_c2_rw1_e8_f0_k1-20_aes_1base_box_aes_mc_bidirectional .
#
...
num_cols=2
field_size=8
translation_name=1base
translation_param=rbase
mixcolumns=bidirectional # Forward and backward translation
rounds=20
keys=20


box_generation="output_rijnsbox_fullcnf_stdname();"
oklib --maxima --batch-string "'print(\"Loading OKlibrary ...\"); oklib_load_all(); num_rows : ${num_rows}; num_columns : ${num_cols}; exp : ${field_size}; final_round_b : false; box_tran : aes_${translation_param}_box; mc_tran : aes_mc_${mixcolumns}; for num_rounds : 1 thru ${rounds} do ( print(\"Generating round \", num_rounds, \" ...\"), output_ss_fcl_std(num_rounds, num_columns, num_rows, exp, final_round_b, box_tran, mc_tran), for seed : 1 thru ${keys} do ( output_ss_random_pc_pair(seed,num_rounds,num_columns,num_rows,exp,final_round_b) )); print(\"Outputting boxes\"); ${box_generation} exit();'" | grep "round\|Loading OKlib\|Outputting" # Using grep to suppress output, but avoid using $ in batch string.

# Generating S-box 1-base
echo "Generating 1-base ..."
QuineMcCluskey-n16-O3-DNDEBUG AES_Sbox_full.cnf > AES_Sbox_PI.cnf
cat AES_Sbox_PI.cnf | RandomShuffleDimacs-O3-DNDEBUG 103 | SortByClauseLength-O3-DNDEBUG > AES_Sbox_PI_sorted.cnf
RUcpGen-O3-DNDEBUG AES_Sbox_PI_sorted.cnf > AES_Sbox_gen.cnf
cat AES_Sbox_gen.cnf | RandomShuffleDimacs-O3-DNDEBUG 1 | SortByClauseLengthDescending-O3-DNDEBUG > AES_Sbox_gen_sorted.cnf
cat AES_Sbox_gen_sorted.cnf | RUcpBase-O3-DNDEBUG > Sbox_8_1base.cnf
